...
static_assert(static_cast<i32>(DDS_DIMENSION_TEXTURE1D) == static_cast<i32>(D3D11_RESOURCE_DIMENSION_TEXTURE1D), "dds mismatch");
static_assert(static_cast<i32>(DDS_DIMENSION_TEXTURE2D) == static_cast<i32>(D3D11_RESOURCE_DIMENSION_TEXTURE2D), "dds mismatch");
static_assert(static_cast<i32>(DDS_DIMENSION_TEXTURE3D) == static_cast<i32>(D3D11_RESOURCE_DIMENSION_TEXTURE3D), "dds mismatch");
static_assert(static_cast<i32>(DDS_RESOURCE_MISC_TEXTURECUBE) == static_cast<i32>(D3D11_RESOURCE_MISC_TEXTURECUBE), "dds mismatch");
...
namespace
{
                                 ...
                                 _Outptr_opt_ ID3D11ShaderResourceView** textureView)
    {
            ...
            (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
        {
            ...
            {
                case D3D11_RESOURCE_DIMENSION_TEXTURE1D:
                    // D3DX writes 1D materials with a fixed Height of 1
        ...
        if (mipCount == 1 && d3dContext && textureView) // Must have context and shader-view to auto generate mipmaps
        {
            ...
            {
                // 10level9 feature levels do not support auto-gen mipgen for volume materials
                                              ...
                                              DDS_ALPHA_MODE* alphaMode)
{
    ...
    if (ddsDataSize < (sizeof(u32) + sizeof(DDS_HEADER)))
    {
        return E_FAIL;
    }

    auto dwMagicNumber = *reinterpret_cast<const u32*>(ddsData);
    if (dwMagicNumber != DDS_MAGIC)
    {
        return E_FAIL;
    }

    auto header = reinterpret_cast<const DDS_HEADER*>(ddsData + sizeof(u32));
                                              ...
                                              DDS_ALPHA_MODE* alphaMode)
{
    ...
    const u32 dwMagicNumber = *reinterpret_cast<const u32*>(ddsData);
    if (dwMagicNumber != DDS_MAGIC)
    {
        return E_FAIL;
    }
                                            ...
                                            DDS_ALPHA_MODE* alphaMode)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (texture || textureView)
        {
            ...
            i32 result = WideCharToMultiByte(CP_ACP,
